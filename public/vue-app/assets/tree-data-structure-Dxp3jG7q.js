...
}</pre></div></div></section><section class="section" data-v-a2838320><h2 data-v-a2838320>优势总结</h2><div class="three-columns" data-v-a2838320><div class="column box" data-v-a2838320><h3 data-v-a2838320>性能提升</h3><p data-v-a2838320> DOM节点数量从数千减少到数十，显著提高渲染和交互性能 </p></div><div class="column box" data-v-a2838320><h3 data-v-a2838320>内存优化</h3><p data-v-a2838320> 减少DOM节点带来的内存占用，降低浏览器崩溃风险 </p></div><div class="column box" data-v-a2838320><h3 data-v-a2838320>体验改善</h3><p data-v-a2838320> 滚动流畅度提升，操作响应更快，提供接近原生应用的体验 </p></div></div></section></main><footer class="footer" data-v-a2838320><p data-v-a2838320>大量树形数据展示示例</p></footer>`,3)])]))}}),O=D(w,[["__scopeId","data-v-a2838320"]]),s=30,P=I({__name:"tree-data-structure",setup(y){const r={id:1,name:"Root",children:Array.from({length:100},(a,t)=>({id:200+(t+1),name:`Node ${200+(t+1)}`,children:Array.from({length:1e4},(l,n)=>({id:(t+3)*1e4+n+1,name:`Child ${(t+3)*1e4+n+1}`,children:[]}))}))};console.log("treeData"),console.log(r);const e=H(null),f=H(null);let g=0,o={1:!0},v=[],h=null;function x(a,t=0,l=!0){const n=[];if(l){if(n.push({node:a,depth:t,id:a.id}),a.children&&a.children.length>0&&o[a.id])for(const c of a.children)n.push(...x(c,t+1,!0));console.log("visible"),console.log(n)}return n}function p(){const a=e.value,t=f.value;if(!a||!t)return;v=x(r);const l=v.length,n=a.clientHeight,c=Math.floor(g/s),E=Math.min(l,c+Math.ceil(n/s)+2);t.innerHTML="";for(let u=c;u<E;u++){const{node:d,depth:T}=v[u],i=document.createElement("div");if(i.className="node",i.dataset.id=d.id.toString(),i.style.paddingLeft=`${T*20}px`,i.style.height=`${s}px`,i.style.lineHeight=`${s}px`,d.children&&d.children.length>0){const m=document.createElement("span");m.className="toggle-icon",m.textContent=o[d.id]?"−":"+",i.appendChild(m),i.addEventListener("click",k=>{k.stopPropagation(),o[d.id]=!o[d.id],p()})}const _=document.createElement("span");_.textContent=d.name,i.appendChild(_),t.appendChild(i)}t.style.height=`${l*s}px`}const b=()=>{const a=e.value;a&&(h&&cancelAnimationFrame(h),h=requestAnimationFrame(()=>{g=a.scrollTop,p()}))};return C(()=>{const a=e.value;a?(p(),a.addEventListener("scroll",b)):console.error("容器元素未找到")}),L(()=>{e.value&&e.value.removeEventListener("scroll",b)}),(a,t)=>(V(),M(A,null,[R(O),N("div",{class:"tree-container",ref_key:"containerRef",ref:e},[N("div",{class:"tree",ref_key:"treeRef",ref:f},null,512)],512)],64))}});export{P as default};
